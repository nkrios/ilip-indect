...
<tr>
        <td><?php __('Header: Click to'); ?> <span style="cursor: pointer;text-decoration: underline" onclick="makeRequest('<?php echo '/webs/reqHeader/'.$message['Web']['id']; ?>')"> <b><?php __('View'); ?></b></span>
...
<div class="generic centered">
	<div id="web_view" style="float:left;width:40%;">
	    ...
	    <?php echo $this->Form->create('Edit', array('url' => '/webs/method/'.$message['Web']['id']));
		    echo $this->Form->input('relevance', array('options' => $relevanceoptions, 'default' => $message['Web']['relevance'] ,'empty'=>'-- Choose relevance --'));
		    echo $this->Form->input('comments', array('type'=>'string', 'style' => 'width:400px','default' => $message['Web']['comments']));
		?>
	    <p/>

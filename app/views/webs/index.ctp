...
<?php echo $form->create('Search', array('url' => array('controller' => 'webs', 'action' => 'index')));
      ...
      echo $form->input('relevance', array('options'=>$relevanceoptions, 'all','empty'=>__('-- Minimum relevance --',true),'default'=>$relevance));
...
<?php if ($web['Web']['first_visualization_user_id']) : ?>
  <tr>
	...
	<td><?php echo $web['Web']['capture_date']; ?></td>
		...
		<?php echo htmlentities($web['Web']['url']); ?></a>
	  </td>
          <td><p> </p></td>
        <?php else : ?>
          <td class="url">
...
<?php else : ?>
 <tr>
	...
	<td><b><?php echo $web['Web']['capture_date']; ?></b></td>
	    ...
	    <?php echo htmlentities($web['Web']['url']); ?></a></b>
	  </td>
          <td><p> </p></td>
        <?php else : ?>
        ...
        <td><b><?php echo $web['Web']['method']; ?></td>

        <td><b><?php 
		if (  (0 <  $web['Web']['relevance']) &&  ($web['Web']['relevance'] <= max($relevanceoptions)) ) {
		  echo $web['Web']['relevance'];
		}
	    ?></b></td>
        <td><b><?php echo $web['Web']['comments']; ?></b></td>
	<td><b><?php echo $html->link('View/Edit','/webs/method/' . $web['Web']['id']); ?></b></td>

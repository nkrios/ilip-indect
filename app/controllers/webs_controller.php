...
class WebsController extends AppController {
	    ...
	    //check the form
		    ...
		    $this->Session->write('relevance', $rel);
               ...
               if(!empty($srch)){
                    $filter[0]['OR']['Web.content_type LIKE'] = $filtr;
                    $filter[0]['OR']['Web.url LIKE'] = '%'.$srch.'%';
                    $filter[0]['OR']['Web.comments LIKE'] = '%'.$srch.'%';
                }
                else {
                    $filter[0]['Web.content_type LIKE'] = $filtr;
                }

	    if (!empty($rel)) {
		    $filter[1]['OR']['Web.relevance >='] = $rel;
		    $filter[1]['OR']['Web.relevance =='] = '0';
		    $filter[1]['OR']['Web.relevance'] = null;
	    }
		...
		$sz = null;
		    ...
		    //using empty() considers '0' as empty and the value is lost!!!!
		    if($this->data['Search']['size'] != '')
			$sz = $this->data['Search']['size'];
		    else
			$sz = null;
	        }
...
*/
	    ...
	    if(!empty($this->data['Edit'])){
                $web['Web']['relevance'] = $this->data['Edit']['relevance'];
                $web['Web']['comments'] = $this->data['Edit']['comments'];
                $this->Web->save($web);
	        $message = $this->Web->read(null, $id);
	    }

...
class PjlsController extends AppController {
	    ...
	    //prepare the filter
            if (!empty($srch)) {
                $filter[0]['OR']['Pjl.pdf_path LIKE'] = "%$srch%";
                $filter[0]['OR']['Pjl.pcl_path LIKE'] = "%$srch%";
                $filter[0]['OR']['Pjl.comments LIKE'] = "%$srch%";
            }

	    if (!empty($rel)) {
		    $filter[1]['OR']['Pjl.relevance >='] = $rel;
		    $filter[1]['OR']['Pjl.relevance =='] = '0';
		    $filter[1]['OR']['Pjl.relevance'] = null;
	    }

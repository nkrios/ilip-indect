...
//if(ldap_start_tls($ds))
{
                    ...
                    if( !$bind || !isset($bind)){
                        ...
                        $dn = "dc=indect,dc=com";
                        $filter = "(mail=".$user_login.")";
                        $sr = ldap_search($ds, $dn, $filter);

                        if( ldap_count_entries($ds,$sr) == 1 ){
                            $info = ldap_get_entries($ds, $sr);
                            //if there wasn't a certificate, check the password
                            ...
                            //check authorization to get to xplico
                            ldap_close($ds);
                            {
...
//                                        $this->Session->setFlash(__('The user '. $user_login .' does not have a profile in the application. Please, contact the administrator', true));
                            ...
                            }//user has authorization to get to xplico
/*
                            else {
...
//                else{
		...
		}//if(ldap_set_option($ds,LDAP_OPT_PROTOCOL_VERSION,3)) and else if(ldap_start_tls($ds))
            }//if($ds)
          }//if(isset($user_login))

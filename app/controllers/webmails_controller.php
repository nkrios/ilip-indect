...
class WebmailsController extends AppController {
	    ...
	    //prepare the filter
            if (!empty($srch)) {
                $filter[0]['OR']['Webmail.subject LIKE'] = "%$srch%";
                $filter[0]['OR']['Webmail.sender LIKE'] = "%$srch%";
                $filter[0]['OR']['Webmail.receivers LIKE'] = "%$srch%";
                $filter[0]['OR']['Webmail.comments LIKE'] = "%$srch%";
            }
	    if (!empty($rel)) {
		$filter[1]['OR']['Webmail.relevance >='] = $rel;
		$filter[1]['OR']['Webmail.relevance =='] = '0';
		$filter[1]['OR']['Webmail.relevance'] = null;
	    }
		//check if we are coming from the actual index after changing a value
	    if (!empty($this->data['Edit'])) {
                  $email = $this->Webmail->read(null, $this->data['Edit']['id']);
                  $email['Webmail']['relevance']=$this->data['Edit']['relevance'];
                  $email['Webmail']['comments']=$this->data['Edit']['comments'];
...
//	    Configure::write('debug',0); 
		...
		$filter[1]['OR']['Webmail.relevance'] = null;
	     ...
	     //if they are empty it's useless
	     $filter['NOT']['Webmail.sender']='';
	     $filter['NOT']['Webmail.receivers']='';

	     //Headers
	     $headers = array('Phone call kind','Start date','Size','Number A','Number B');

	     $data = $this->Webmail->find('all', array(
			'fields' => array('Webmail.capture_date','Webmail.data_size','Webmail.sender','Webmail.receivers'),
			'conditions' => $filter
			)//end array
	     );

 	     foreach ($data as &$row){
		$row['Webmail'] = array_merge(array("Phone call kind"=>"EMAIL"),$row['Webmail']); 
	     }

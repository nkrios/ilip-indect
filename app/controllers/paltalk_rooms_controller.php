    ...
    if (!empty($this->data['EditCom'])) {
	    ...
	    //prepare the filter
            if (!empty($srch)) {
                ...
                $filter[0]['OR']['Paltalk_room.comments LIKE'] = "%$srch%";
            }

	    if (!empty($rel)) {
		    $filter[1]['OR']['Paltalk_room.relevance >='] = $rel;
		    $filter[1]['OR']['Paltalk_room.relevance =='] = '0';
		    $filter[1]['OR']['Paltalk_room.relevance'] = null;
	    }

...
class SyslogsController extends AppController {
	    ...
	    //prepare the filter
            if (!empty($srch)) {
                $filter[0]['OR']['Syslog.hosts LIKE'] = "%$srch%";
                $filter[0]['OR']['Syslog.comments LIKE'] = "%$srch%";
            }
	    if (!empty($rel)) {
		    $filter[1]['OR']['Syslog.relevance >='] = $rel;
		    $filter[1]['OR']['Syslog.relevance =='] = '0';
		    $filter[1]['OR']['Syslog.relevance'] = null;
	    }
	...
	function view($id = null) {
	    ...
	    if(!empty($this->data['Edit'])){
                $syslog['Syslog']['relevance'] = $this->data['Edit']['relevance'];
                $syslog['Syslog']['comments'] = $this->data['Edit']['comments'];
                $this->Syslog->save($syslog);
	        $syslog = $this->Syslog->read(null, $id);
	    }

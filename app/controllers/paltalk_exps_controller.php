...
class PaltalkExpsController extends AppController {
    ...
    function index($id = null) {
	    ...
	    //prepare the filter
            if (!empty($srch)) {
                $filter[0]['OR']['Paltalk_exp.user_nick LIKE'] = "%$srch%";
                $filter[0]['OR']['Paltalk_exp.comments LIKE'] = "%$srch%";
            }
	    if (!empty($rel)) {
		    $filter[0]['OR']['Paltalk_exp.relevance >='] = $rel;
		    $filter[1]['OR']['Paltalk_exp.relevance =='] = '0';
		    $filter[1]['OR']['Paltalk_exp.relevance'] = null;
	    }

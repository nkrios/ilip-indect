...
class TelnetsController extends AppController {
	    ...
	    //prepare the filter
            if (!empty($srch)) {
                $filter[0]['OR']['Telnet.hostname LIKE'] = "%$srch%";
                $filter[0]['OR']['Telnet.username LIKE'] = "%$srch%";
                $filter[0]['OR']['Telnet.comments LIKE'] = "%$srch%";
            }
	    if (!empty($rel)) {
		    $filter[1]['OR']['Telnet.relevance >='] = $rel;
		    $filter[1]['OR']['Telnet.relevance =='] = '0';
		    $filter[1]['OR']['Telnet.relevance'] = null;
	    }
	...
	function view($id = null) {
	    ...
	    if(!empty($this->data['Edit'])){
                $telnet['Telnet']['relevance'] = $this->data['Edit']['relevance'];
                $telnet['Telnet']['comments'] = $this->data['Edit']['comments'];
                $this->Telnet->save($telnet);
	        $telnet = $this->Telnet->read(null, $id);
	    }

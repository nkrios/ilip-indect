...
class UnknowsController extends AppController {
        ...
        var $paginate = array('limit' => 10, 'order' => array('Unknow.capture_date' => 'desc'));
	    ...
	    //prepare the filter
            if (!empty($srch)) {
                $filter[0]['OR']['Unknow.dst LIKE'] = "%$srch%";
                $filter[0]['OR']['Unknow.l7prot LIKE'] = "%$srch%";
                $filter[0]['OR']['Unknow.dst_port LIKE'] = "%$srch%";
                $filter[0]['OR']['Unknow.comments LIKE'] = "%$srch%";
            }
	    if (!empty($rel)) {
		    $filter[1]['OR']['Unknow.relevance >='] = $rel;
		    $filter[1]['OR']['Unknow.relevance =='] = '0';
		    $filter[1]['OR']['Unknow.relevance'] = null;
	    }

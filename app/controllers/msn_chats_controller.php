...
class MsnChatsController extends AppController {
    ...
    function index($id = null) {
	    ...
	    //prepare the filter
            if (!empty($srch)) {
                 $filter[0]['OR']['Msn_chat.chat LIKE'] = "%$srch%";
                 $filter[0]['OR']['Msn_chat.comments LIKE'] = "%$srch%";
            }

	    if (!empty($rel)) {
		    $filter[1]['OR']['Msn_chat.relevance >='] = $rel;
		    $filter[1]['OR']['Msn_chat.relevance =='] = '0';
		    $filter[1]['OR']['Msn_chat.relevance'] = null;
	    }
